...
from env import Env
from utils.ranking_policy import sql_order_clause_for
...
class RDBProc:
    ...
    def insert_game_record(self, record: GameRecord) -> int:
        insert_query = text("""
            ...
            (game_name, level, user_uuid, nickname, clear_time, clear_time_ns, score, mistake_count, hint_count, is_verified, user_ip)
            VALUES
            (:game_name, :level, :user_uuid, :nickname, :clear_time, :clear_time_ns, :score, :mistake_count, :hint_count, :is_verified, :user_ip)
        """)
        params = {
    ...
    def get_ranking(self, game_name: str, level: str, limit: int = 10) -> list[GameRecord]:
        order_clause = sql_order_clause_for(game_name)
